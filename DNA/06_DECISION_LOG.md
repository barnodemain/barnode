...
| 2026-06-28 | **Fix bottom-nav PWA**: `.bottom-nav` con `align-items: flex-start` e `.bottom-nav-inner` con `height: var(--nav-height)` (non più `100%`) | In PWA installata le icone erano centrate nello spazio totale (nav-height + safe-area) → fascia verde vuota sotto le scritte. Ora le icone stanno nell'area utile e la safe-area resta padding sotto. Desktop invariato. |
